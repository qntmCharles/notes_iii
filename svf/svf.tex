...
\usepackage{joshkirklin,pgfplots,cancel}
...
\begin{eg}
	...
	$\dunder{\sigma_2} \cdot \bm{n} - \dunder{\sigma_1} \cdot \bm{n} = \gamma
	(\nabla \cdot \bm{n})\bm{n}$. We consider $\y \in \partial V$ to find the
	interfacial motion:
	\begin{enumerate}
		\item $\lambda = 1$: we have 
			\begin{equation}
				\u(\y) = -\gamma \int_{\partial V} (\nabla \cdot
				\bm{n})\bm{n}\cdot\dunder{J}(\y-\x)\,\diffd S_x
			\end{equation}
		...
		\item $\lambda \ne 1$: to solve the integral equation
			\begin{equation}
				\frac{\lambda + 1}{2} \u(\y) = -\gamma \int_{\partial V}
				(\nabla \cdot \bm{n})\bm{n}\cdot\dunder{J}\,\diffd S_x +
				(\lambda -1) \int_{\partial V} \u(\x) \cdot
				\dunder{\underline{K}} \cdot \bm{n} \, \diffd S_x
			\end{equation}
			for $\u$, we discretize the integrals and invert the resulting
...
we are interested in the flow $\u$ in $V$, we imagine a flow $\u^*$ in $V^*$
with $\mu^* = \mu$ and $\u^* = \u$ on $\partial V$. Such a flow exists if
$\int_{\partial V} \u \cdot \bm{n} \, \diffd S = 0$.
\begin{center}
	\begin{tikzpicture}[scale=0.4]
		\draw[fill=gray] plot[smooth, tension=.7] coordinates {(-3.5, 0.5) (-3,
		2.5) (-1,3.5) (1.5,3) (4,3.5) (5,2.5) (5,0.5) (2.5,-2)
		(0,-1.5) (-3,-2) (-3.5,0.5)};
		\draw[thick,->] (1.5,3) -- (3, 4.5) node[right] {$\bm{n}$};
		\draw (1,1) node {$V^*$};
		\draw (7,2) node {$V, \mu$};
	\end{tikzpicture}
\end{center}
...
\begin{equation}
	\u(\y) = \int_{\partial V} \dunder{J}(\y-\x) \cdot \bm{f}_S(\x) \, \diffd
	S
\end{equation}
where $\bm{f}_S = (\dunder{\sigma}^* - \dunder{\sigma})\cdot\bm{n}$ is the
Stokeslet density. Note:
\begin{enumerate}
	\item $\dunder{\sigma}^*$ may not be the real stress in $V^*$ unless the
		body really is fluid with $\mu = \mu^*$.
	\item If the body is rigid then $\u^*$ is rigid-body motion and
		$\dunder{\sigma}^* = \bm{0}$. In this case $\bm{f}_S$ is the stress
		$-\dunder{\sigma}\cdot\bm{n}$ exerted by the body on the fluid. The
		external flow depends \emph{only} on $\dunder{\sigma}\cdot\bm{n}$ and
		\emph{not} on $\u$.
\end{enumerate}
...
$\bm{V}(s) = \dot{\bm{X}}$ in background flow $\u^\infty(\x)$. 
...
\begin{center}
	\begin{tikzpicture}[scale=2]
		...
		\draw (3, 0) node {centreline $\bm{X}(s,t)$};
...
\begin{equation}
	\u(\y) = \u^\infty(\y) + \int_{-L}^L \dunder{J}(\y - \bm{X}(s)) \cdot
	\bm{f}(s) \, \diffd s
\end{equation}

\lecture{30/10/20}
Note the integral is the first term in a far-field expansion for a ring of
Stokeslets. Consider a point $\y = \X(s_0) + \varepsilon \bm{R}(s_0,\theta)$
...
\begin{align}
	\bm{f}(s) &\approx \bm{f}(s_0) \\
	\y - \X(s) &\approx \X'(s_0) (s_0-s)
\end{align}
Hence we have
\begin{equation}
	\bm{V}(s_0) = \u^\infty(\X(s_0)) + \frac{1}{8\pi \mu} \left( \dunder{I} +
		\X'(s_0) \X'(s_0)\right) \cdot \bm{f}(s_0) \left[ \int_L^{s_0 -
...
\begin{eg}
	...
	\begin{center}
		\begin{tikzpicture}
			...
			\draw[thick,->] (0,0) -- (0,-1) node[right] {$\bm{V}$};
