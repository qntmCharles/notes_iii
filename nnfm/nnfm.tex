...
\begin{equation}
	\mathcal{L} \equiv P + \int_S \lambda \nabla \cdot \bm{u} \, \diffd V
\end{equation}
...
To generalise this to GNF, consider the power density $f = \eta \srate^2$ for
a Newtonian fluid. Analogously, for a GNF we define
\begin{equation}
	f = \int_0^{\srate} \eta(x)x\, \diffd x
\end{equation}
and a new Lagrangian
\begin{equation}
	\mathcal{L} = \mathcal{L}\left[\bm{u},\lambda\right] = \int_V f \, \diffd
	V + \int_V \lambda \nabla \cdot \bm{u}\,\diffd V
\end{equation}

\subsubsection{Calculus of variations}
Consider a functional $J$ of a vector field $\bm{y}(\bm{x})$ and of its
spatial derivative. 
\begin{equation}
	J\left[\bm{y}\right] = \int_V F(x_i,y_j,y_{m,n}) \, \diffd V
\end{equation}
where $y_{m,n} \equiv \frac{\partial y_m}{\partial x_n}$ and $V$ is the total
domain. The first variation of $J$ is $\delta J$ where
\begin{equation}
...
Now $\delta J = 0$ for all $\delta y$ implies $\delta F = 0$ for all $\delta
y$, giving the \emph{Euler-Lagrange equation}.
\begin{equation}
...
The Euler-Lagrange equation for $\lambda $ gives
\begin{equation}
...
\begin{align}
	...
	&= \frac{1}{2\srate} \frac{\partial}{\partial u_{j,n}}\left(
u_{p,q}u_{p,q} + u_{p,q}u_{q,p}\right) \\
...
\begin{align}
	\frac{\partial}{\partial x_n} \left( \frac{\partial F-2}{\partial u_{j,n}}
		\right) &= \frac{\partial}{\partial x_n} \left( \lambda
		\frac{\partial}{\partial u_{j,n}} \frac{\partial u_i}{\partial x_i}
	\right) \\
...
this threshold a \emph{yield-stress}, denoted by $\sigma_y$. Fluids with such
a property are \emph{yield-stress fluids} or \emph{viscoplastic fluids}.
...
Typical flow problem of yield-stress fluids separates the domain into two
regions.
\begin{enumerate}
...
yielding close to these boundaries.
...
\end{equation}
We wish to find where the fluid yields. Clearly near $r=0$ the stress is small
so the fluid is unyielded. The yield surface is defined by $\abs{\sigma} =
\sigma_y$ thus is defined by
\begin{equation}
...
\begin{enumerate}
	...
	\item if $r_y < R$ (equivalently $\Delta p > \frac{2L}{R} \sigma_y$) the
		fluid yields in $r_y \le r \le R$, and is unyielded is in $0 \le r \le
		r_y$.
\end{enumerate}
...
In the unyielded region we have plug flow $u = \text{const.}$. For $0 \le r
\le r_y$: velocity $u = U_y = u_{\text{yielded}}(r_y)$ by continuity.
\begin{equation}
...
The flow rate $Q$ vanishes if $r_y \ge R$. For $r_y \le R$, 
\begin{align}
...
difficult if the yield surface has unsteady motion, in which case inertia is
needed.
