...
\usepackage{joshkirklin,pgfplots,ulem}
...
time. $G$ has dimensions of stress. By causality, $G(s) = 0$ so that future
changes in shear rate do not influence the present stress.
...
deviatoric stress
\begin{equation}
	\dunder{\tau}(t) = \int_{-\infty}^t G(t-t') \dunder{\srate}(t')\,\diffd t'
\end{equation}
Note that $G$ is still a scalar.

\paragraph{Steady flow.} Consider a steady flow with $\dunder{\srate}(t') =
\dunder{\srate}$ a constant tensor. Then
\begin{equation}
	\dunder{\tau}(t) = \dunder{\srate} \int_{\infty}^t G(t-t') \, \diffd t' =
	\left[ \int_0^\infty G(s) \, \diffd s \right] \dunder{\srate}
\end{equation}
which is the Newtonian constitutive relationship. Thus if the flow is steady,
stress is Newtonian with $\dunder{\tau} = \eta \dunder{\srate}$ and $\eta =
...
$\bm{u} = u(y) \hat{\bm{x}}$. Then from the Cauchy equation we have
\begin{equation}
