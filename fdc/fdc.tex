...
incompressibility $\nabla \cdot \symbf{u} = 0$, the velocity vector is perpendicular
to $\symbf{k}$. Thus $\theta$ is also the angle that fluid parcel trajectories
...
\begin{enumerate}
	\item Assuming the buoyancy frequency $N$ is constant and $\symbf{f}$ is
	...
	\item Given $L \gg D$, the Coriolis force may be neglected in the vertical
		...
		\begin{equation}
			\symbf{f} \times \symbf{f} = \symbf{f}_h \times \symbf{u}_h + \symbf{f}_v \times
			\symbf{u}_h + \symbf{f}_h \times \symbf{u}_v + \cancel{\symbf{f}_v \times
			\symbf{u}_h} \approx \symbf{f}_h \times \symbf{u}_h + \symbf{f}_v \times
			\symbf{u}_h
		\end{equation}
		where we have assumed $\abs{\symbf{u}_v} \ll \abs{\symbf{u}_h}$. At low
		latitude, this assumption fails since $\symbf{f}_v \ll \symbf{f}_h$.
		Following the traditional approximation, only the $\symbf{f}_v \times
		\symbf{u}_h$ contribution is retained in the horizontal momentum
		equation. This is equivalent to replacing $\symbf{f}$ with its vertical
		component only.
...
zero, and $\nabla \cdot \symbf{u}_g = 0$. We also assume that the $y$-lengthscale
$L_y$ is sufficiently small that $\beta L_y \ll f_0$. Then if $\Ro \ll 1$, it
follows that $\abs{\symbf{u}_a} \ll \abs{\symbf{u}_g}$. Thus we are domain limited
in latitude.
...
\begin{align}
	...
	\nabla\right] (u_g + u_a) - f_0 (v_g + v_a) - \beta y (v_g + v_a) &=
	-\frac{1}{\rho_0} \frac{\partial \tilde{p}}{\partial x} \label{eq:prim1}\\
	\left[ \frac{\partial}{\partial t} + (\symbf{u}_g+\symbf{u}_a)\cdot
	\nabla\right] (v_g + v_a) + f_0 (u_g + u_a) + \beta y (u_g + u_a) &=
...
\paragraph{Approximation validity.} Given that $\Ro \ll 1$ we may approximate
$\frac{\diffD \symbf{u}}{\diffD t}$ by $\frac{\diffD_g \symbf{u}_g}{\diffD t}$ where
$\frac{\diffD_g}{\diffD t} = \partial_t + \symbf{u}_g \cdot \nabla$, and also
$\beta y \symbf{u}$ can be approximated by $\beta y \symbf{u}_g$. 

Note that $f_0(u_a, v_a)$ and $\beta y(u_g, v_g)$ are of similar size. Hence
the requirement $\beta L_y / f_0 \ll 1$ is better expressed by $\beta L_y/f_0
\sim \Ro$. Also, $w_a \frac{\diffd \rho_s}{\diffd z}$ is retained, but $w_a
\frac{\diffd \tilde{\rho}}{\diffd z}$ is not, which requires $\abs{\diffd
\tilde{\rho}/\diffd z} \ll \abs{\diffd \rho_s / \diffd z}$. Denoting the
horizontal scale as $L$ and the vertical scale as $D$, the thermal wind
equation gives $g \tilde{\rho}/L \rho_0 \sim f_0 U/D$ where $U$ is the typical
horizontal velocity scale. Hence we have
\begin{equation}
	\frac{\tilde{\rho}_z}{\rho_{s,z}} \sim \frac{f_0 U L \rho_0}{gD^2
	\rho_{s,z}} = \frac{U}{f_0 L} \left( \frac{L f_0}{N D}\right)^2 = \Ro \Bu
\end{equation}
where $\Bu \equiv (L f_0 / D N)^2$ is the \emph{Burger number}. For our
approximation to be valid, we require $\Ro \Bu \ll 1$. If $\Bu \sim 1$, then
this is implied by $\Ro \ll 1$.  
...
\begin{equation}
	...
	\psi_z}{N^2}\right)_z \right] + \beta \psi_x = 0
\end{equation}
