...
\lecture{2/11/20}
\section{Stratification and internal gravity waves}
...
$-p_0'(z) = \rho_0 g$, and the remainder $p'$. The vertical component of the
...
It iss further useful to write $\rho'(x,y,z,t) = \rho_s (z) +
\tilde{\rho}(x,y,z,t)$ where $\rho_s(z)$ is a \emph{background} or
\emph{reference} density and $\tilde{\rho}$ is the \emph{dissturbance
...
\subsection{Stratification in the atmosphere and ocean}
...
\subsection{Small-ampltiude motion about a state of rest}
...
\begin{equation}
	\nabla^2 \tilde{w}_tt + N^2(\tilde{w}_{xx} + \tilde{w}_{yy}) = 0
\end{equation}
...
\end{equation}
which gives $\bm{c}_g \cdot \bm{k} = 0$, i.e. the group velocity is
perpendicular to phase surfaces.
