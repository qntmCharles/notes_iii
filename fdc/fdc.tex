...
\end{equation}
which gives $\bm{c}_g \cdot \bm{k} = 0$, i.e. the group velocity lies in
...
follows that $\abs{\bm{u}_a} \ll \abs{\bm{u}_g}$. Thus we are domain limited
in latitude.

\lecture{6/11/20}
The primitive equations may now be written
\begin{align}
	\left[ \frac{\partial}{\partial t} + (\bm{u}_g+\bm{u}_a)\cdot
...
$\frac{\diffD \bm{u}}{\diffD t}$ by $\frac{\diffD_g \bm{u}_g}{\diffD t}$ where
$\frac{\diffD_g}{\diffD t} = \partial_t + \bm{u}_g \cdot \nabla$, and also
$\beta y \u$ can be approximated by $\beta y \u_g$. 
...
prognostic equation, we eliminate $\u_a$ by taking the curl of horizontal
...
\begin{itemize}
	\item At rigid side boundaries, we require the normal component of $\u$ is
